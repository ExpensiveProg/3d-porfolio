...
// window.scrollTo(0,0); this iws going to scroll to the top of the page
const Navbar = () => {
  ...
  return (
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`} >
      ...
      <div className="w-full flex justify-between items-center max-w-7xl mx-auto ">
        ...
        <p className='text-white text-[18px] font-bold cursor-pointer flex ' >Azeez &nbsp; <span className='sm:block hidden'>| Kamolideen</span> </p>
        </Link>
        ...
        <div className="sm:hidden flex flex-1 justify-end items-center">
          ...
          { navLinks.map((link) => (
            <li key={link.id} className={`${active === link.title ? "text-white" : "text-secondary"} font-poppins font-medium cursor-pointer text-[16px]  `} onClick={() => {
              // this function means that when we click on any of those link it should close the dropdown menu
              setToggle(!toggle);
